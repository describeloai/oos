...
| `OOS4009` | **reservado** | capacidad solicitada por una función y no concedida. `Function` está aplazado a v1alpha2 |
| `OOS4010` | **reservado** | acción invocable por agentes sin `requiresApproval` ni límite. `Function` está aplazado a v1alpha2 |
| `OOS2001` | **reservado** | *«referencia a un nombre cualificado inexistente»*. Al poblar la suite se vio que **no es alcanzable**: toda referencia de v1alpha1 tiene código específico — entidad y propiedad (`OOS2005`), datasource (`OOS2004`), etiqueta (`OOS4003`). Se reserva porque `Rule`, `Function` y `Test` introducirán tipos de referencia nuevos en v1alpha2 |
