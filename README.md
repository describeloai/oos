...
| [`conformance/v1alpha4/`](conformance/v1alpha4/README.md) · borrador | ✅ **24/24** |
| **listo para v1** | ◐ **9 de 12 estaciones** — [`00-scope`](spec/v1alpha4/00-scope.md) §8 |
...
**Y 24/24 no quiere decir terminado.** Un `kind` atraviesa doce estaciones —despacho, forma,
referencias, tipos, flujo, gobierno, significado, forma canónica, sellado, compatibilidad,
emisión y dependencia— y `Property` e `Interface` pasan **nueve**: lo que queda es que una
propiedad con `is` se emite **sin tipo y sin clasificación**, y que importar un concepto de
...
los códigos de v1alpha1 sin tocar, porque un concepto declara lo mismo que una propiedad. El criterio de «listo» **nunca había estado escrito** —por eso cada borrador
